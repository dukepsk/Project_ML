...
ada = AdaBoostClassifier(base_estimator=classifier,
                         n_estimators=500, 
                         learning_rate=0.1,
                         random_state=1)
...
sc = cross_validate(estimator=ada,
                        X=data_train,
                        y=target_train,
                        cv=2,
                        scoring=scorings,
...
y_pred = ada.predict(data_test)
y_proba = ada.predict_proba(data_test)
...
data = {'clf': 'Ada (Test)', 'ACC': ACC, 'PRE': PRE, 'REC': REC, 'F1': F1, 'ROCAUC': ROCAUC}
df = df.append(data, ignore_index=True)
df = df.set_index(['clf'])
